...
Detecta artefactos, carga checklists JSON, simula verificación
y genera reporte Markdown. En modo dry_run NO crea tickets
reales en Jira ni páginas en Confluence.
...
@dataclass
class ChecklistItem:
    """Ítem individual de una checklist."""
...
class ArtifactDetector:
    ...
    def detect(self) -> list[DetectedArtifact]:
        """Escanea documentacion/ y directorios de código."""
        artifacts: list[DetectedArtifact] = []

        # 1. Documentación (PDFs)
        if DOCUMENTACION_DIR.exists():
            for pdf in sorted(DOCUMENTACION_DIR.glob("*.pdf")):
        ...
        # 2. Código fuente (Java / Vue)
        code_dirs = [PROJECT_ROOT / "src", PROJECT_ROOT / "biblioteca-frontend"]
        code_found = False
        ...
        if code_found:
            ...
            logger.info("Artefacto detectado: Código fuente → codigo.json")
...
class ChecklistLoader:
    ...
    def load(self, checklist_file: str) -> Checklist:
        ...
        path = CHECKLISTS_DIR / checklist_file
        if not path.exists():
...
class DryRunSimulator:
    """Simula la verificación de checklist basada en evidencia conocida."""

    def simulate(self, checklist: Checklist) -> Checklist:
        """Para cada ítem, determina resultado a partir de evidence_location."""
...
class ReportGenerator:
    ...
    def generate(
        ...
        report_path = REPORTES_DIR / f"{timestamp}_wf4_reporte.md"

        lines: list[str] = []
        lines.append("# Reporte de Inspección Estática — Workflow 4: Orquestador de Quality Gates\n")
        lines.append(f"**Fecha de ejecución:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        lines.append(f"**Modo:** `{'DRY RUN' if DRY_RUN else 'PRODUCCIÓN'}`\n")
        ...
        for art in artifacts:
            ...
            lines.append(f"**Estándar:** {checklist.standard}\n")
            lines.append(f"**Versión Checklist:** {checklist.version}\n\n")

            # Tabla de resultados
            lines.append("| ID | Categoría | Ítem | Resultado | Severidad | Evidencia |\n")
            lines.append("|---|---|---|---|---|---|\n")
            for item in checklist.items:
            ...
            lines.append(f"- **Total ítems:** {total}\n")
            ...
            lines.append(f"- **Cobertura de Revisión:** {self._pct(cumplen + parciales, total)}\n\n")
        ...
        # Sección de Dry Run
        lines.append("---\n")
        lines.append("## 3. Acciones de Quality Gate\n")
        if DRY_RUN:
            lines.append(
            ...
            lines.append(
                "> **PRODUCCIÓN:** Los defectos han sido sincronizados con Jira/Confluence.\n\n"
            )

        # Cierre
        lines.append("---\n")
        lines.append("*Generado automáticamente por WF4 Orquestador de Quality Gates — SQA Equipo 11*\n")
...
class WF4Orchestrator:
    ...
    def run(self) -> Path:
        ...
        if DRY_RUN:
            logger.info("[DRY RUN] No se crearán tickets en Jira ni páginas en Confluence")
        else:
            logger.info("[PRODUCCIÓN] Sincronización con Jira/Confluence habilitada")
        ...
        for art in artifacts:
            try:
                ...
                logger.info(
                    "Checklist %s: %d ítems, %d defectos",
                    art.checklist_file,
                    len(checklist.items),
        ...
        # 3. Generar reporte
        report_path = self.reporter.generate(artifacts, results)
